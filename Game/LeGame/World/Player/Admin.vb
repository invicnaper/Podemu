        ...
        Public Shared Sub Execute(ByVal Client As GameClient, ByVal Command As String, ByVal Parameters() As String, ByVal Parameter As String)
            ...
            Try
                ...
                Select Case Command
                    ...
                    Case "learnjob"
                        Game.JobManager.SendPlayerjob(Client)
                        Client.SendNormalMessage("Vous venez d'apprendre le metier")
                    Case "addpts"
                        If ParamNum >= 2 Then

                            Dim FromClient As GameClient = CharactersManager.CharactersList(Parameters(1)).Client
                            If Not FromClient Is Nothing Then
                                Client.Infos.Points += Parameters(0)
                                Client.Character.Save()
                                Client.ConsoleMessage(Parameters(0) & " points ajoutes a " & Parameters(1))
                            Else
                                Client.ConsoleMessage("Player '" & Parameters(1) & "' doesn't exist.", 0)
                            End If
                        End If
                    Case "remplirehdv"
                        HdvManager.fullingHDV()
                        Client.ConsoleMessage("HDVS recharger !")
                    Case "kick"
                        If ParamNum >= 1 Then
                            Dim KickClient As GameClient = World.Players.GetCharacter(Parameters(0))
                            If Not KickClient Is Nothing Then
                    ...
                    Case "infos"
                        If Config.GetItem("MODE_LIGHT") = "False" Then
                            Client.ConsoleMessage("---------------------------------")
                            Client.ConsoleMessage("Podemu rev 0.3.0 codé by invic")
                            ...
                            If Config.GetItem("MODE_LIGHT") = "True" Then
                                Client.ConsoleMessage("---------------------------------")
                                Client.ConsoleMessage("PodLight rev 0.3.0101.1 codé by invic")
                                Client.ConsoleMessage(Players.GetPlayerCount & " players connected to game server.")
                                Client.ConsoleMessage("Uptime : " & Basic.GetUptime)
                                Client.ConsoleMessage("---------------------------------")
                            End If
                    ...
                    Case "abo"
                        If Client.Infos.GmLevel = 20 Then
                            ...
                            Dim UpdateString As String = "subscriptionDate=@subscriptionDate"
                            Dim SQLText As String = "UPDATE `player_accounts` SET `subscriptionDate` = " & Now.ToString & " WHERE `id` = " & Client.Infos.Id & ";"
                            Dim SQLCommand As New MySqlCommand(SQLText, Sql.Accounts)
                            Dim P As MySqlParameterCollection = SQLCommand.Parameters

                            P.Add(New MySqlParameter("@subscriptionDate", Now.ToString))

                            SQLCommand.ExecuteNonQuery()

                            Client.SendNormalMessage("Le joueur <b>" & Client.Character.Name & "</b> est maintenant abo <b>" & Now.ToString & "</b>.")
                        End If
                    Case "mail"

                        SyncLock Sql.AccountsSync
                            Dim mail As String = Client.Infos.SubscriptionTime
                            Dim SQLText As String = "UPDATE player_accounts SET mail=" & mail & " WHERE id=" & Client.Infos.Id
                            Dim SQLCommand As New MySqlCommand(SQLText, Sql.Accounts)
                            SQLCommand.Parameters.Add(New MySqlParameter("@mail", mail))
